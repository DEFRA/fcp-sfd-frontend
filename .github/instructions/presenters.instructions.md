...
- **Pure function** — no side effects, no DAL or session access. Signature `(data, payload) => ({ … })`; the primary data parameter is named **`data`** (not `personalDetails` / `businessDetails`).
...
- Direct-GET fallback chain so a page renders when there's no pending session change: `payload ?? data.change<Field> ?? data.<field>`, where `data.<field>` is the **flat, mapper-provided value** (e.g. `data.userName`, `data.businessEmail`) — see mappers conventions. Use `?? null` for optional display fields.

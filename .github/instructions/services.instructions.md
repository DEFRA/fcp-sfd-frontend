...
- Keep services free of Hapi request/response plumbing (no `h`, no `request`, no view rendering). **Pass only the specific values the service uses** — never whole `credentials` or `params` objects (see the 'pass only what's used' rule in routes).
...
- Build the mutation `variables` via the shared **build-update-variables util** (engine target — today `build-<domain>-update-variables-service`), rather than an inline `{ input: { … } }` in each service. Run mutations through `updateDalService(mutation, variables, <auth>)` — never call the DAL connector directly from a feature service.
- After a successful mutation: **clear the session** (`yar.clear('personalDetailsUpdate')`), then `flashNotification(yar, 'Success', <message>)`.
- **Success messages come from the shared catalogue in the engine** (used by personal + business across both services), not inline strings.
- **Clear the session, don't get-and-reset it.** Do not read-then-rewrite the session bag or re-set ids (e.g. SBI) back into session — the record id comes from the route `params` instead (see routes). This follows the personal-details pattern.
...
- **Data access differs by service — do not unify.** This (external) service authenticates DAL calls with `sessionId` (from Defra ID); internal uses the user `email` (header gateway). Pass whichever value the service needs (plus ids like `sbi`) — not the whole credentials object — and keep the two auth models separate.

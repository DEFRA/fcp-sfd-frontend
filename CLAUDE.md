...
Frontend service for the Single Front Door (SFD) on DEFRA's Future Farming and Countryside Programme. It's a Hapi.js server with Nunjucks templates using GOV.UK Frontend, serving farmers and land managers to view/manage their business and personal details. Data flows through the DAL (fcp-dal-api) via GraphQL to Rural Payments (KITS) upstream services.
...
- **Auth**: Defra ID via OpenID Connect through `@hapi/bell`. Token refresh handled by SSO plugin. Routes use Hapi auth strategies.
- **Feature Toggles**: Boolean env vars checked via `src/config/feature-toggle.js` (convict). Currently: interrupter journeys, CPH data.

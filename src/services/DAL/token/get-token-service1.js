...
const getTokenService = async (cache) => {
  console.log('Inside getTokenService')
  return retry(() => getToken(cache))
}

/**
 * Attempts to get a token from cache first.
 * If no cached token exists, requests a new one from the DAL.
 */
const getToken = async (cache) => {
  console.log('Inside getToken')
  // Try to get token from cache to avoid unnecessary API calls
...
/**
 * Performs the actual POST request to the DAL token endpoint.
